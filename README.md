...
- __demo_prsem.all_tr_features__: features for all the isofroms to derive and assign pRSEM prior. The first line is a header and the rest is one isoform per line. The description for each columns are:
...
- __demo_prsem.all_tr_prior__: prior parameters for all the isoforms. This file does not have a header. Each line contains a prior parameter and an isoform's transcript ID seperated by "  # ".
- __demo_uniform_prior_1.gene.results__: RSEM's posterior mean estimates on the gene level with a uniform prior parameter in value one. 
- __demo_uniform_prior_1.isoform.results__: RSEM's posterior mean estimates on the isoform level with a uniform prior parameter in value one. 
